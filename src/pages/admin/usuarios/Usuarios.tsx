  ...
  const fetchUsuarios = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        const usuariosFormatados = (data || []).map((item: any) => ({
            id: item.id,
            nome: item.nome,
            email: item.email,
            tipo: item.tipo,
            ...
            termosCriados: item.termos_criados || 0,
            termosAssinados: item.termos_assinados || 0,
            videosEnviados: item.videos_enviados || 0,
      ...
      setStats({
        ...
        pendentes: usuariosFormatados.filter(u => u.status === "pendente").length,
        bloqueados: usuariosFormatados.filter(u => u.status === "bloqueado").length,
        totalTermos: usuariosFormatados.reduce((acc, u) => acc + u.termosCriados + u.termosAssinados, 0),
        totalVideos: usuariosFormatados.reduce((acc, u) => acc + u.videosEnviados, 0)
      });
    } catch (error) {
